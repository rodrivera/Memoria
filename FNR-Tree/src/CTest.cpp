...
void readEdges(const char *filename, map<long, pair<int,int> > *nodes, CTree* tree){
	ifstream infile(filename);
	string line;
	while(getline(infile,line)){
		istringstream iss(line);
...
void readBrinkhoff(const char *filename, CTree* tree){
...
void readQueries(const char *inFilename, const char *outFilename, CTree* tree){
